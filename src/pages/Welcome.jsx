...
export default function WelcomePage() {
  ...
  const cityY = useTransform(scrollY, [0, 400], [0, -150]);
  const cityOpacity = useTransform(scrollY, [0, 200, 300, 500], [1, 0.5, 0.5, 0]);

  const heroY = useTransform(scrollY, [0, 400], [0, -300]);
  const heroOpacity = useTransform(scrollY, [0, 300, 500], [1, 1, 0]);

  const textY = useTransform(scrollY, [0, 200, 300, 500], [0, 50, 50, 400]);
  const textScale = useTransform(scrollY, [0, 300], [1, 1.5]);

  return (
    <>
      <header id="welcome-header">
        <motion.div id="welcome-header-content" style={{ scale: textScale, y: textY }}>
